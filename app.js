...
console.log("Start Fetching Website");

const linkList = new Set();
const visitedLinks = new Set();
const MAX_DEPTH = 2;
const DELAY = 1000; // 1 seconds delay
...
async function processLink(link, depth, retries = 0) {
    try {
        ...
        // Save link with title and description
        saveLinkInfo(link, title, description);
...
function saveLinkInfo(link, title, description) {
    ...
    // Combine new link info with existing data
    const linkInfo = { link, title, description };
    const allLinks = Array.from(new Set([...existingData, linkInfo]));
...
scratch("https://www.edenschool.fr/");
keepAlive();
